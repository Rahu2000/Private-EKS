...
export SNAPSHOT_ENABLE="false" # [true|false]
...
if [ -z "$IAM_POLICY_ARN" ]; then
  IAM_POLICY_ARN=$(aws iam create-policy --policy-name ${IAM_POLICY_NAME} --policy-document file://./templates/ebs-csi-policy.json | jq -r .Policy.Arn)
fi
...
helm upgrade --install ${RELEASE_NAME} \
  ./../helm-charts/aws-ebs-csi-driver \
  --set fullnameOverride=${RELEASE_NAME} \
  --set image.repository=602401143452.dkr.ecr.${REGION}.amazonaws.com/eks/aws-ebs-csi-driver \
  --set sidecars.provisioner.image.repository=${REPO_ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${REPO_PREFIX}/csi-provisioner \
  --set sidecars.attacher.image.repository=${REPO_ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${REPO_PREFIX}/csi-attacher \
  --set sidecars.snapshotter.image.repository=${REPO_ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${REPO_PREFIX}/csi-snapshotter \
  --set sidecars.livenessProbe.image.repository=${REPO_ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${REPO_PREFIX}/livenessprobe \
  --set sidecars.resizer.image.repository=${REPO_ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${REPO_PREFIX}/csi-resizer \
  --set sidecars.nodeDriverRegistrar.image.repository=${REPO_ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${REPO_PREFIX}/csi-node-driver-registrar \
