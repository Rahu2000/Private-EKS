...
helm upgrade --install ${RELEASE_NAME} \
  ./../helm-charts/aws-efs-csi-driver \
  --set fullnameOverride=${RELEASE_NAME} \
  --set image.repository=602401143452.dkr.ecr.${REGION}.amazonaws.com/eks/aws-efs-csi-driver \
  --set sidecars.csiProvisionerImage.repository=${REPO_ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${REPO_PREFIX}/csi-provisioner \
  --set sidecars.livenessProbeImage.repository=${REPO_ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${REPO_PREFIX}/livenessprobe \
  --set sidecars.nodeDriverRegistrarImage.repository=${REPO_ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${REPO_PREFIX}/csi-node-driver-registrar \
