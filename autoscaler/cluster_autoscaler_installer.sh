...
if [ "Darwin" == "$LOCAL_OS_KERNEL" ]; then
  ...
  sed -i '' "s|REGION|${REGION}|g" /tmp/${DIR}/cluster-autoscaler.values.yaml
...
helm upgrade --install ${RELEASE_NAME} \
  ...
  --set image.repository=${REPO_ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${REPO_PREFIX}/cluster-autoscaler \
